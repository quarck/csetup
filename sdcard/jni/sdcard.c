        ...
        const struct fuse_in_header *hdr, const struct fuse_setattr_in *req)
{
	...
	// 0 - access, 1 - modification

	times[0].tv_sec = sb.st_atime;
	times[0].tv_usec = sb.st_atime_nsec / 1000;

        times[1].tv_sec = sb.st_mtime;
        times[1].tv_usec = sb.st_mtime_nsec / 1000;
        ...
        if (req->valid & FATTR_ATIME) {
	      ...
	      times[0].tv_usec = now.tv_usec;
              ...
              times[0].tv_usec = req->atimensec / 1000;
            }
        }
        if (req->valid & FATTR_MTIME) {
            if (req->valid & FATTR_MTIME_NOW) {
              ...
              times[1].tv_usec = req->mtimensec / 1000;
            }
        }
...
static int usage()
{
    ERROR("usage: sdcard [-t<threads>] <source_path> <uid> <gid>\n"
...
int main(int argc, char **argv)
{
    ...
    const char *dest_path = "/storage/sdcard0";
    uid_t uid = 0;
    gid_t gid = 0;
